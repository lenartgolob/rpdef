...
def get_top_players():
    ...
    try:
        mydb = connect_to_db()
        mycursor = mydb.cursor(dictionary=True)
        mycursor.execute(f"SELECT id, player, team, position, gp, min, pts, reb, ast, tov, stl, blk, ROUND(rdef, 2) as rdef, ROUND(pdef, 2) as pdef, ROUND(def, 2) as def, NbaPlayerId FROM player WHERE SeasonYear = %s ORDER BY DEF DESC LIMIT 20", (current_year,))
        player = mycursor.fetchall()
        mydb.close()
        mycursor.close()
        return jsonify(player)
...
def get_season():
    try:
        ...
        mycursor.execute(f"SELECT id, player, team, position, gp, min, pts, reb, ast, tov, stl, blk, ROUND(rdef, 2) as rdef, ROUND(pdef, 2) as pdef, ROUND(def, 2) as def, NbaPlayerId FROM player WHERE SeasonYear = %s ORDER BY DEF DESC", (year,))
        player = mycursor.fetchall()
        mydb.close()
        mycursor.close()
        return jsonify(player)
